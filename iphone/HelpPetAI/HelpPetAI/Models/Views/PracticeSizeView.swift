...
struct PracticeSizeView: View {
    ...
    @State private var selectedPracticeType: String?
    @Environment(\.colorScheme) var colorScheme
    @Environment(\.dismiss) private var dismiss
    
    private let fullText = "What type of practice do you work at?"
    private let typingSpeed: Double = 0.015
    
    private let practiceOptions = [
        ("🏠", "Solo practice (1 location)", "solo"),
        ("👨‍👩‍👧‍👦", "Family/Mom-Pop owned", "family"),
        ("🏢", "Private practice (multiple locations)", "private_multi"),
        ("🏛️", "Corporate owned", "corporate")
    ]
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // Character and Dialog (centered layout like response screens)
                    VStack(spacing: 32) {
                        ...
                        VStack(spacing: 20) {
                            ...
                            ZStack {
                                ...
                                .frame(width: 280, height: 92) // Height includes tail
                    ...
                    if showOptions {
                        VStack(spacing: 16) {
                            ForEach(practiceOptions.indices, id: \.self) { index in
                                let option = practiceOptions[index]
                                
                                Button(action: {
                                    selectedPracticeType = option.2
                                    // Add haptic feedback
                                    ...
                                    HStack(spacing: 16) {
                                        ...
                                        // Selection indicator
                                            if selectedPracticeType == option.2 {
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 12)
                                            .stroke(selectedPracticeType == option.2 ? Color.green : optionButtonBorder, lineWidth: selectedPracticeType == option.2 ? 2 : 1)
                                    )
                                    .clipped()
                                    .shadow(
