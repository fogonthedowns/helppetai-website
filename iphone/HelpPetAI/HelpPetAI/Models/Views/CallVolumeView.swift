...
struct CallVolumeView: View {
    ...
    @State private var selectedCallVolume: String?
    @Environment(\.colorScheme) var colorScheme
    @Environment(\.dismiss) private var dismiss
    
    private let fullText = "About how many phone calls does your practice receive daily?"
    private let typingSpeed: Double = 0.015
    
    private let callVolumeOptions = [
        ("📱", "10-25 calls per day", "10-25"),
        ("☎️", "25-50 calls per day", "25-50"),
        ("📞", "50-100 calls per day", "50-100"),
        ("🔥", "100-200 calls per day", "100-200"),
        ("🚨", "200+ calls per day", "200+")
    ]
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 32) {
                        ...
                        VStack(spacing: 20) {
                            ...
                            ZStack {
                                ...
                                .frame(width: 300, height: 92) // Height includes tail
                    ...
                    if showOptions {
                        VStack(spacing: 16) {
                            ForEach(callVolumeOptions.indices, id: \.self) { index in
                                let option = callVolumeOptions[index]
                                
                                Button(action: {
                                    selectedCallVolume = option.2
                                    // Add haptic feedback
                                    ...
                                    HStack(spacing: 16) {
                                        ...
                                        // Selection indicator
                                        if selectedCallVolume == option.2 {
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 12)
                                            .stroke(selectedCallVolume == option.2 ? Color.green : optionButtonBorder, lineWidth: selectedCallVolume == option.2 ? 2 : 1)
                                    )
                                    .shadow(
